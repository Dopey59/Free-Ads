...
<head>
    ...
    <title>Poster une annonce | FreeAds</title>
</head>
<body class="">
    <!-- NAV BAR-->
    <header>
    ...
    <div class="adsTitle flex justify-center p-4">
        <h1 class="font-bold text-blue-900 text-2xl">{ Créez votre annonce }</h1>
    </div>
    <x-guest-layout>
        <!-- Session Status -->
    <x-auth-session-status class="mb-14" :status="session('status')" />
        <div class="boxForm w-full m-auto mb-2">
            ...
            <form method="POST" action="{{ route('adStore') }}">
                @csrf
                ...
                <div class="mt-4">
                    <x-input-label for="img" :value="__('Image 1 (insérez l url)')" />
